...
class CountryAgent(mesa.Agent):
    ...
    def step(self):
       ...
       self.alert_adoption_threshold = self.alert_adoption_threshold + (self.alert_adoption_threshold + 0.00002*np.exp(0.05*len(list_of_lockdown_countries)) - self.alert_adoption_threshold)
...
for j in range(no_of_iterations):
        ...
        model = CountryModel(0.012, 0.125, 15, 'real', 'yes')
        for i in range(31):
                        ...
                        print(str(i)+" "+str(model.time))                                    
                        if model.data_update == "yes" and i % 2 == 0:
                            for agent in model.schedule.agents:
                                 agent.state = lockdown_data_for_calibration_micro[(agent.code==lockdown_data_for_calibration_micro.Code) & (model.time == lockdown_data_for_calibration_micro.model_step)]["lockdown"].iloc[0]
                        else: 
                            pass
