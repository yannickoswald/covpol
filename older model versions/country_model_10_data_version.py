...
#%%
## Fake? (ex-post) Particle Filter for conference example only


n_of_filterings_per_step = int(no_of_iterations * 0.1)
...
def create_fanchart(arr):
    ...
    ax.plot(df_results_particle_filtered[(df_results_particle_filtered.iteration == 0) & (df_results_particle_filtered.AgentID == 0)]["Step"], 
             lockdown_data_for_calibration_agg[0]*100,
             linewidth=3 ,label = "data", linestyle= "--", color = "tab:red")
...
## example df.loc[df['Buyer'] == 'Tom', 'Price'] = 0
#df_results_filtered.loc[df_results_filtered['iteration'] == to_be_filtered[0] , 'Lockdown'] = df_results_filtered[(df_results_filtered.iteration == sample_arr[0][0])]["Lockdown"].to_numpy()
#df_results_filtered.loc[df_results_filtered['iteration'] == to_be_filtered[0] , 'minimum_difference'] = df_results_filtered[(df_results_filtered.iteration == sample_arr[0][0])]["minimum_difference"].to_numpy()
#df_results_filtered.loc[df_results_filtered['iteration'] == to_be_filtered[0] , 'adoption_mode'] = df_results_filtered[(df_results_filtered.iteration == sample_arr[0][0])]["adoption_mode"].to_numpy()
...
# test whether sample() returns uniform distribution.
#sample_arr = np.zeros((1000,1))
#for i in range(1000):
 #  sample_arr[i,0] =  sample(sample_from_indices_arr.tolist() ,1)[0]
