...
### makes the model run time vastly slower
def compare_lockdown_agent_to_data_micro_level(model):
    
     agent_lockdown_states = np.array([agent.state for agent in model.schedule.agents])
     real_data = np.array([(lockdown_data_for_calibration_micro[
                   (lockdown_data_for_calibration_micro.Code == agent.code) 
                 & (lockdown_data_for_calibration_micro.model_step == model.time)]["lockdown"])
                  for agent in model.schedule.agents])[:,0]

     ### if close to 1 high validity, close to 0 low validity (at a specific time step)
     return np.mean(agent_lockdown_states == real_data)
...
class CountryAgent(mesa.Agent):
    ...
    def step(self):
       ...
       self.alert_adoption_threshold = self.alert_adoption_threshold + (self.alert_adoption_threshold + 0.00002*np.exp(0.042*len(list_of_lockdown_countries)) - self.alert_adoption_threshold)
...
class CountryModel(mesa.Model):
    ...
    def __init__(self, base_alert, social_base_threshold, clique_size, initial_conditions):
        ...
        self.datacollector = mesa.DataCollector(
            ...
            model_reporters={
                             ...
                             "micro_validity": compare_lockdown_agent_to_data_micro_level,
                             },
...
for j in range(no_of_iterations):
        ...
        model = CountryModel(0.015, 0.12, 16, 'real')
        for i in range(31):
                        model.step()
...
for i in range(no_of_iterations):
    ax1.plot( df_results[(df_results.iteration == i) & (df_results.AgentID == 0)]["Step"] + 1,
              df_results[(df_results.iteration == i) & (df_results.AgentID == 0)]["Number_of_lockdowns"] / Num_agents * 100, alpha = 0.5)
ax1.plot(df_results[(df_results.iteration == i) & (df_results.AgentID == 0)]["Step"] + 1, 
...
ax1.text(18, 5, "Initial conditions " + str(model.initial_conditions))
plt.show()
...
for i in range(Num_agents):  
        ...
        if len(model_lockdown_data) > 0 and len(real_world_lockdown_date) > 0:
            ...
            ax3.scatter(difference,np.repeat(i,len(difference)), color = "tab:blue", alpha = 0.2)
        print("iteration of micro-level-plot is " + str(i))
ax3.set_xlabel("diff. in days")   
ax3.set_ylabel("country index")  
ax3.plot([0,0],[0,175], color = "black", linewidth = 3)
ax3.margins(0)
plt.show()
...
for i in range(31):
    array_run_results[:,i] = df_results_filtered["Number_of_lockdowns"].iloc[i::31]
...
for i in range(no_of_iterations):
    ax5.plot(df_results_filtered[(df_results.iteration == i)]["Step"]+1, df_results_filtered[(df_results.iteration == i)]["micro_validity"])
ax5.set_xlabel("Day of March")
ax5.set_ylabel("Micro validity")
#%%
