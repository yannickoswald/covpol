...
for i in range(31):
    ...
    globals()['ax11%s' % i].hist(np.array(df_results_filtered[((df_results_filtered.Step == i))]["Number_of_lockdowns"]), bins = int(no_of_iterations/10))
...
for i in range(no_of_iterations):
       micro_validity_metric_array[:,i] = np.mean(np.array_split(np.array(pd.Series.reset_index(df_results[(df_results.iteration == i) ]["Lockdown"], drop = True) == pd.Series.reset_index(lockdown_data_for_calibration_micro.sort_values(['model_step', 'Entity'])["lockdown"],drop = True)),31), axis = 1)
