...
class CountryAgent(mesa.Agent):
    ...
    def step(self):
       ...
       self.alert_adoption_threshold = self.alert_adoption_threshold + (self.alert_adoption_threshold + 0.000015*np.exp(0.04*len(list_of_lockdown_countries)) - self.alert_adoption_threshold)
...
def create_fanchart(arr):
    ...
    ax.legend(['Mean'] + [f'Pct{2*o}' for o in offsets] + ['data'], frameon = False)
    return fig, ax
