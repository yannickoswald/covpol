...
if __name__ == '__main__':    
    ...
    micro_validity_list = [y[2] for y in data_results]
    mean_micro_validity_list = [sum(y)/len(y) for y in zip(*micro_validity_list)]    
    
    sum_of_mse_over_time = sum(np.array(mse_as_list))
    print("The sum of mse over time is ", sum_of_mse_over_time)
    
    average_micro_validity_over_time = np.mean(np.array(micro_validity_list))
    print("The average micro validity over time is ", average_micro_validity_over_time)
