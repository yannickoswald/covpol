...
class CountryAgent(mesa.Agent):
    ...
    def compute_distance(self):
       ...
       if self.state == 0:
           ...
           index_for_choice = 15
           self.minimum_difference = np.mean(total_differences_array[0:index_for_choice])
...
class CountryModel(mesa.Model):
    ...
    def __init__(self, base_alert, social_base_threshold):
        ...
        for i in range(self.num_agents):
            ...
            #### by themselves
            a.alert_adoption_threshold = agent_data["log_population_density_normalized_on_average"][i] * base_alert
...
for i in range(no_of_iterations):
    plt.plot( df_results[(df_results.iteration == i) & (df_results.AgentID == 0)]["Step"] + 1,
              df_results[(df_results.iteration == i) & (df_results.AgentID == 0)]["Number_of_lockdowns"] / 172 * 100)
...
for i in range(no_of_iterations):
    ...
    average_min_and_max_diff = np.zeros((2,1))
    for j in range(31):
