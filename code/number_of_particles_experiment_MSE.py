...
if __name__ == '__main__':
    ...
    ### per particle number
    lockdown_data1_list = [lockdown_data1] * \
        len(num_power_particles_list)*iterations_filter
    lockdown_data2_list = [lockdown_data2] * \
        len(num_power_particles_list)*iterations_filter

    number_of_particles_per_experiment = [
        2**x for x in num_power_particles_list]
    results_mse_all = []
    results_msepf_all = []
    results_micro_all = []
    ...
    iterations_as_columns = np.linspace(
        1, iterations_filter, iterations_filter)
    start = datetime.now()
    ...
    for num_power_particles in num_power_particles_list:
        ...
        inputs_for_starmap = sorted(
            list(zip(pool_input, lockdown_data1_list, lockdown_data2_list)))
        ...
        with Pool(processes=number_of_processors) as pool:
            data_results = list(pool.starmap(
                Experiment.run_experiment, inputs_for_starmap))
       # assert len(data_results) == len(num_power_particles_list), f"The length of the results isn't what we expected {len(num_power_particles_list)}"
        ...
        print(
            f"Completed the iterations with  {2**num_power_particles} particles")
