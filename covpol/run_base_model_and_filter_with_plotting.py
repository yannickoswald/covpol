...
for i in tqdm(range(no_of_iterations)):
    array_run_results[i, :] = np.sum(np.split(
        np.array(df_results[(df_results.iteration == i)]["Lockdown"]), 31), axis=1)
...
if no_of_iterations >= 10 and no_of_iterations <= 50:
    ...
    for i in range(31):
        ...
        globals()['ax11%s' % i].hist(
            array_run_results[:, i], bins=int(no_of_iterations/10))
    ...
    plt.savefig('Distribution of runs over each time step.png',
...
if no_of_iterations <= 200:
    ...
    for i in range(no_of_iterations):
        ...
        for j in range(31):
            average_min_diff_array[j, 0] = np.mean(
                df_iter[(df_iter.Step == j)]["minimum_difference"])
            average_min_and_max_diff[j, 0] = min(
                df_iter[(df_iter.Step == j)]["minimum_difference"])
            average_min_and_max_diff[j, 1] = max(
                df_iter[(df_iter.Step == j)]["minimum_difference"])

        ax2.plot((df_results[(df_results.iteration == i) & (
            df_results.AgentID == 0)]["Step"] + 1)[1:30], average_min_diff_array[1:30])
...
if no_of_iterations <= 50:
    ...
    for i in tqdm(range(Num_agents)):
        model_lockdown_data = df_results[(
            df_results.code == agent_data["code"][i]) & (df_results.Lockdown == 1)]
        real_world_lockdown_date = lockdown_data2[(
            lockdown_data2.Code == agent_data["code"][i]) & (lockdown_data2.lockdown == 1)]
        for p in range(no_of_iterations):
            model_lockdown_data2 = model_lockdown_data[(
                model_lockdown_data.iteration == p)]["Step"]
            if len(model_lockdown_data2) > 0 and len(real_world_lockdown_date) > 0:
                difference = model_lockdown_data2.iloc[0] - (
                    int(real_world_lockdown_date.iloc[0]["Day"][0:2])-1)
...
df_differences_per_country = pd.DataFrame(data=micro_validity_BIG.T,
                                          ...
                                          columns=np.linspace(
                                              0, no_of_iterations-1, no_of_iterations).astype(int),
...
df_differences_per_country = pd.concat([df_differences_per_country,
                                        pd.DataFrame(
                                            np.array(agent_data["gdp_pc"]), index=agent_data.code),
                                        pd.DataFrame(np.array(agent_data["democracy_index"]), index=agent_data.code)], axis=1)

df_differences_per_country_sorted = df_differences_per_country.sort_values(by=[
                                                                           'error'])
...
if no_of_iterations < 50:
    ...
    for i in range(164):
        ax30.scatter(np.repeat(i, len(df_differences_per_country_sorted.iloc[i, 0:no_of_iterations])),
                     df_differences_per_country_sorted.iloc[i,
                                                            0:no_of_iterations],
...
for i in range(no_of_iterations):
    alpha = pd.Series.reset_index(
        df_results[(df_results.iteration == i)]["Lockdown"], drop=True)
    beta = pd.Series.reset_index(lockdown_data2.sort_values(
        ['model_step', 'Entity'])["lockdown"], drop=True)
    micro_validity_metric_array[:, i] = np.mean(
        np.array_split(np.array(alpha == beta), 31), axis=1)
