...
lockdown_data_for_calibration_micro = pd.read_csv('lockdown_tracking.csv', 
                                                encoding = 'unicode_escape')
covid_cases_normalized = pd.read_csv('covidcases31st.csv', 
                                                encoding = 'unicode_escape')
...
class CountryAgent(mesa.Agent):
    ...
    def step(self):
       ...
       nudge_final = (self.alert_adoption_threshold + a*np.exp(b) - self.alert_adoption_threshold)
           
       self.alert_adoption_threshold = self.alert_adoption_threshold + nudge_final
...
squared_error_macro_level = ((mean_model_runs/164 - lockdown_data_for_calibration_agg[0].to_numpy())**2)*100
...
### role-mode? https://github.com/Urban-Analytics/dust/blob/main/Projects/ABM_DA/stationsim/particle_filter_gcs.py
...
n_of_filterings_per_step = int(no_of_iterations * percentage_filtered/100)
da_window = 10
...
for i in range(No_of_particles):
        ### call the model iteration
        ##4th parameter initial conditions can be real, no countries yet or random
        current_model = copy.deepcopy(CountryModel(0.01, 0.13, 18, 'real', 'no'))
...
def error_particle_obs(particle):
    ### returns the error between particle and real-world
    ### parameters 
    ...
    data5 = lockdown_data_for_calibration_micro[(lockdown_data_for_calibration_micro.model_step == t)]["lockdown"]
...
data1 = list_of_particles[0].datacollector.get_agent_vars_dataframe()
data2 = (data1.reset_index(level=0)).reset_index(level = 0)
data3 = data2[(data2.Step == list_of_particles[0].time)]
data4 = pd.Series.reset_index(data3["Lockdown"], drop = True)
data5 = lockdown_data_for_calibration_micro[(lockdown_data_for_calibration_micro.model_step 
                                     == list_of_particles[0].time)]["lockdown"]
data6 = pd.Series.reset_index(data5, drop = True)
particle_validity = np.mean(data4 == data6)

pd.Series.reset_index(lockdown_data_for_calibration_micro[(lockdown_data_for_calibration_micro.model_step == (t-1))]["lockdown"], drop = True)


pd.Series.reset_index(data2[(data2.Step == (t-1))]["Lockdown"], drop = True) == pd.Series.reset_index(lockdown_data_for_calibration_micro[(lockdown_data_for_calibration_micro.model_step == (t-1))]["lockdown"], drop = True)
